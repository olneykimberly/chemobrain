...
saveToPDF(paste0("../../results_", projectID, "/sex_stratified/MDS/MDS_raw_sex.pdf"), width = 6, height = 6)
```
...
# save
path <- (paste0("../../results_", projectID, "/sex_stratified/JSD/JSD_raw"))
...
write.table(lcpm_gene_names, paste0("../../results_", projectID, "/sex_stratified/counts/filtered_lcpm_gene_names.txt"), sep= "\t", row.names = FALSE, quote=FALSE)
...
write.table(lcpm_gene_names, paste0("../../results_", projectID, "/sex_stratified/counts/filtered_cpm_gene_names.txt"), sep= "\t", row.names = FALSE, quote=FALSE)
```
...
saveToPDF(paste0("../../results_", projectID, "/sex_stratified/MDS/MDS_filtered_condition.pdf"), width = 6, height = 6)
```
...
# save
path <- (paste0("../../results_", projectID, "/sex_stratified/library/gene_density"))
...
path <- (paste0("../../results_", projectID, "/sex_stratified/library/lcpm_boxplots"))
...
table(dge.filtered.norm$samples$Group_Sex)
formula <- (~ 0 + Group_Sex)
...
path <- paste0(paste0("../../results_", projectID, "/sex_stratified/voom/voom_raw"))
...
```{r varpart}
form_varPart <- ~ (1|Group_Sex) + RIN + Ttr + Hbb + Omp
...
saveToPDF(paste0("../../results_", projectID, "/sex_stratified/variance/variance_partition_violins.pdf"), width = 8.5, height = 5)
...
write.table(variance_explained, paste0("../../results_", projectID, "/sex_stratified/variance/variance_explained.tsv"),sep = "\t", quote = FALSE, row.names = FALSE)
```
...
```{r}
form <- ~ Group_Sex + RIN + Ttr + Hbb + Omp
# Compute Canonical Correlation Analysis (CCA) # between all pairs of variables
# returns absolute correlation value
C = canCorPairs(form, sorted_info)
# Plot correlation matrix
plotCorrMatrix( C )
saveToPDF(paste0("../../results_", projectID, "/sex_stratified/variance/CCA_with_project.pdf"), width = 8, height = 8)
...
sumTable
write.table(sumTable, paste0("../../results_", projectID, "/sex_stratified/DEGs/DEGs_summary_q", qval,"_log2FC", lfc.cutoff, ".txt"), quote = FALSE, sep = "\t")
...
pdf(paste0("../../results_", projectID, "/sex_stratified/DEGs/DEGs_heatmap.pdf"), height = 6, width = 6)
...
Female_MTX_vs_SALINE <- topTable(
  veBayesFit, 
  coef = "Female_MTX_vs_SALINE",  
  n = Inf, 
  p.value = 1,
  lfc = 0, 
  sort.by = "P", 
...
for (i in allComparisons) {
  ...
  path <- paste0("../../results_", projectID, "/sex_stratified/DEGs/", i, "_q1.00.txt", sep = "") 
  ...
  vTopTableSig <-
    topTable( 
      ...
      genelist = genes_relevant, 
      confint = TRUE)
  path <- paste0("../../results_", projectID, "/sex_stratified/DEGs/", i, "_q", qval, "_lfc", lfc.cutoff,".txt", sep = "") 
...
ggplot(data = pcaWithMetadata, aes(x = PC1, y = PC2, shape = Group, color = Group)) +
  geom_point(size = 2.5) +
  theme_bw() 
saveToPDF(paste0("../../results_", projectID, "/sex_stratified/PCA/PCA_dim1&2.pdf"), width = 6, height = 5)

ggplot(data = pcaWithMetadata, aes(x = PC1, y = PC2, shape = Sex, color = Sex)) +
  geom_point(size = 2.5) +
  theme_bw() 
saveToPDF(paste0("../../results_", projectID, "/sex_stratified/PCA/PCA_dim1&2_Sex.pdf"), width = 6, height = 5)

ggplot(data = pcaWithMetadata, aes(x = PC2, y = PC3, shape = Group, color = Group)) +
  geom_point(size = 2.5) +
  theme_bw() 
saveToPDF(paste0("../../results_", projectID, "/sex_stratified/PCA/PCA_dim2&3.pdf"), width = 6, height = 5)

write.table(pcaWithMetadata, paste0("../../results_", projectID, "/sex_stratified/PCA/PCA_metadata.txt"), quote = FALSE, sep = "\t")
```
...
for (i in allComparisons) {
  group1_vs_group2 <-
    read.delim(paste0("../../results_", projectID, "/sex_stratified/DEGs/", i,"_q1.00.txt"))
    i <- gsub("_", " ", i)
    ...
    i <- gsub(" ", "_", i)
    pdf(paste0("../../results_", projectID, "/sex_stratified/volcano/", i, ".pdf"), height = 6, width = 6)
    print(p)
    dev.off()
  } else {
  color_values <- vector()
...
for (i in allComparisons) {
  filepath <- paste0("../../results_", projectID, "/sex_stratified/DEGs/", i,"_q1.00.txt")
  assign(paste0(i),
...
write.xlsx(list_of_datasets, file = paste0("../../results_", projectID, "/sex_stratified/DEGs/DEGs.q1.00.xlsx"))
```

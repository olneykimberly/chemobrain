---
title: "Differential expression - chemobrain"
...
# add Sample_ID counts name to the counts files
...
row.names(count_data) <- count_data$V1
count_data$V1 <- NULL
# set column names to Sample_ID counts file name
...
all.equal(colnames(count_data), (metadata$Sample_ID)) # counts columns match the Sample_ID IDs
```
...
dge$samples$Group <- factor(dge$samples$Group, levels = c("SALINE", "LEUC", "MTX", "MTXLEUC")) # 
condition_colors <- c("skyblue", "lightpink","darkgreen", "gray30")[dge$samples$Group]
...
The TMM method estimates scale factors between Sample_IDs that can be incorporated 
...
# For each Sample_ID plot the lcpm density
for (i in 1:nSample_IDs){
...
# clean up
rm(log2cpm.tech, log2cpm.norm, log2cpm.filtered, den, colors, nSample_IDs, path, i)
```

# Expression check 
```{r}
info <- as.data.frame(dge.filtered.norm$samples)
info$sample <- info$Sample_ID
...
#info <- extract_gene_cpm("Gh", cpm_gene_names, info)
#info <- extract_gene_cpm("Prl", cpm_gene_names, info)

# Gather the expression for the four genes into a long format for easy plotting
plot_data <- info %>%
  dplyr::select(Sample_ID, Group, Uty, Omp, "Hbb-bs", Ttr) %>%
  tidyr::gather(key = "Gene", value = "Expression", Uty, Omp, "Hbb-bs", Ttr) %>%
  dplyr::mutate(
    # Order the groups for plotting
    group = factor(Group, levels = c("SALINE", "LEUC", "MTX", "MTXLEUC")),
    # Order the samples (Sample_ID) based on the factor order of the group
    Sample_ID = factor(Sample_ID, levels = unique(Sample_ID[order(group)])), # **<--- KEY UPDATE**
...
for (g in genes_to_plot) {
  p <- plot_data %>%
    dplyr::filter(Gene == g) %>%
    ggplot(aes(x = Sample_ID, y = Expression, fill = group)) +
    geom_col() +
    labs(
      title = paste(g),
    ...
    scale_fill_manual(values = c("SALINE" = "skyblue", "LEUC" = "lightpink", "MTX" = "darkgreen", "MTXLEUC" = "gray30")) +
    theme_minimal() +
    theme(
...
path_plot <- paste0("../results_", projectID, "/both_sexes/gene_expression_checks/Uty_Omp_Hbb_Ttr_Gh_Prl_expression.pdf")
ggsave(path_plot, combined_plot, width = 18, height = 6)
...
Sample_ID_order <- colnames(voomCounts)
sorted_info <- info[match(Sample_ID_order, info$Sample_ID), ]
...
colnames(design) <- c("SALINE", "LEUC", "MTX", "MTXLEUC", "Sex", "RIN")
# Inspect
design
```

# Voom
When the library sizes are quite variable between Sample_IDs, then the voom approach is theoretically more powerful than limma-trend. 
...
v <- voomWithQualityWeights(
  ...
  plot = TRUE # should mean-variance trend & Sample_ID-specific weights be displayed?
  )
```
...
pheatmap::pheatmap(df,
                   ...
                   fontsize_number = 12,
                   number_color = ifelse(df > 50, "white", "black"),
                   angle_col = "0",
                   breaks = c(seq(0, 100, length.out = length(rev(natparks.pals("WindCave"))))))
dev.off()
```
...
# Adding metadata
dim1_10$Sample_ID <- rownames(dim1_10)
sorted_info$Sample_ID <- sorted_info$Sample_ID
pcaWithMetadata <- merge(dim1_10, sorted_info, by = "Sample_ID", all = TRUE)
...
```{r}
# Sex as a covariate
# 1. MTX_vs_SALINE DEGs (adj.P.Val <= 0.1)
MTX_vs_SALINE_up <- subset(MTX_vs_SALINE, logFC > 0.0 & adj.P.Val <= 0.1)
MTX_vs_SALINE_down <- subset(MTX_vs_SALINE, logFC < -0.0 & adj.P.Val <= 0.1)

# 2. MTXLEUC_vs_SALINE DEGs (adj.P.Val <= 0.1)
MTXLEUC_vs_SALINE_up <- subset(MTXLEUC_vs_SALINE, logFC > 0.0 & adj.P.Val <= 0.1)
MTXLEUC_vs_SALINE_down <- subset(MTXLEUC_vs_SALINE, logFC < -0.0 & adj.P.Val <= 0.1)

# 3. LEUC_vs_SALINE DEGs (adj.P.Val <= 0.1)
LEUC_vs_SALINE_up <- subset(LEUC_vs_SALINE, logFC > 0.0 & adj.P.Val <= 0.1)
LEUC_vs_SALINE_down <- subset(LEUC_vs_SALINE, logFC < -0.0 & adj.P.Val <= 0.1)
```

# UpSet
```{r}
# Create a list of gene sets (using row names as gene IDs)
gene_sets <- list(
    # Upregulated Genes
    MTX_Up = MTX_vs_SALINE_up$gene_name,
    MTXLEUC_Up = MTXLEUC_vs_SALINE_up$gene_name,
    LEUC_Up = LEUC_vs_SALINE_up$gene_name,
    
    # Downregulated Genes
    MTX_Down = MTX_vs_SALINE_down$gene_name,
    MTXLEUC_Down = MTXLEUC_vs_SALINE_down$gene_name,
    LEUC_Down = LEUC_vs_SALINE_down$gene_name
)

# Convert the list to the UpSetR-compatible binary data frame
upset_data <- fromList(gene_sets)

# 1. Define the desired order for the sets (Up then Down)
# This list is based on the names in your gene_sets list.
ordered_sets <- c(
    "MTX_Up", "LEUC_Up", "MTXLEUC_Up",
    "MTX_Down", "LEUC_Down", "MTXLEUC_Down"
)

# 2. Define the colors for the set size bars (Up = Red, Down = Blue)
# The order MUST match the 'ordered_sets' vector above.
set_colors <- c(
    "red", "red", "red",     # Colors for Up sets
    "blue", "blue", "blue"   # Colors for Down sets
)

# Generate the UpSet plot with updated parameters
upset(
    upset_data,
    nsets = 6,
     keep.order = T,
    ...
    mainbar.y.label = "Intersection Size (Number of Genes)",
    sets.x.label = "Set Size (Total Genes)",
    # Scale text sizes (e.g., c(main.bar.title, main.bar.ticks, set.size.title, set.size.ticks, set.names, numbers.above.bars))
    text.scale = c(1.3, 1.2, 1, 1, 1.5, 1.2),
    show.numbers = TRUE
)

# save
path <- paste0("../results_", projectID, 
               "/both_sexes/upset/UpSet_DEGs_q0.1")
